...
def install():
    if is_conda_env():
        ...
        try:
            result = subprocess.run(conda_cmd, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            print(result.stdout.decode())
        except subprocess.CalledProcessError as e:
            print(f"Error during conda installation: {e.stderr.decode()}")
        ...
        try:
            result = subprocess.run(pip_cmd, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            print(result.stdout.decode())
        except subprocess.CalledProcessError as e:
            print(f"Error during pip installation: {e.stderr.decode()}")
            sys.exit("## Torchserve/Model archiver Installation Failed!")
