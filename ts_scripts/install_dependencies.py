...
class Linux(Common):
    ...
    def install_neuronx_driver(self):
        ...
        subprocess.run([self.sudo_cmd, "tee", "/etc/apt/sources.list.d/neuron.list"], input=f"deb https://apt.repos.neuron.amazonaws.com {os.environ['VERSION_CODENAME']} main\n", text=True, check=True)
        subprocess.run(["wget", "-qO", "-", "https://apt.repos.neuron.amazonaws.com/GPG-PUB-KEY-AMAZON-AWS-NEURON.PUB"], check=True)
        subprocess.run([self.sudo_cmd, "apt-key", "add", "-"], check=True)
