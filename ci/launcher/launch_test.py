...
def launch_ec2_instance(region, instance_type, ami_id):
    ...
    try:
        ...
        with ec2_connection.cd("/home/ubuntu"):
            ...
            ec2_connection.run("sudo apt-get install -y python3.8-dev python3.8-distutils python3.8-venv")
            ...
            ec2_connection.run(
                "curl -O https://bootstrap.pypa.io/get-pip.py && python get-pip.py", warn=True
            )
        ...
        if any(command_return_value_map.values()):
            raise ValueError("*** One of the commands executed on ec2 returned a non-zero value.")
        else:
            LOGGER.info("*** All commands executed successfully on ec2. command:return_value map is as follows:")
